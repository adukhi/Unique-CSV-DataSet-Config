...
public class UniqueCSVDataSetBeanInfo extends BeanInfoSupport {
    ...
    private static final String BLOCK_SIZE = "blockSize";            //$NON-NLS-1$
    private static final String THREAD_GROUP = "threadGroup";        //$NON-NLS-1$

    // Access needed from CSVDataSet
    private static final String[] SHARE_TAGS = new String[4];
    static final int SHARE_ALL    = 0;
    static final int SHARE_GROUP  = 1;
    static final int SHARE_THREAD = 2;
    static final int SHARE_TARGET = 3;
    ...
    static {
        ...
        SHARE_TAGS[SHARE_TARGET] = "shareMode.target"; //$NON-NLS-1$
    }

    
    public UniqueCSVDataSetBeanInfo() {
        super(UniqueCSVDataSet.class);

        createPropertyGroup("unique_csv_data",             //$NON-NLS-1$
                new String[] { FILENAME, FILE_ENCODING, VARIABLE_NAMES,
                        IGNORE_FIRST_LINE, DELIMITER, QUOTED_DATA,
                        RECYCLE, STOPTHREAD, SHAREMODE, THREAD_GROUP, BLOCK_SIZE});
        ...
        p = property(THREAD_GROUP);
        p.setValue(NOT_UNDEFINED, Boolean.TRUE);
        p.setValue(DEFAULT, "Thread Group A");        //$NON-NLS-1$
        p.setValue(NOT_EXPRESSION, Boolean.FALSE);
    }
