...
public class UniqueCSVDataSet extends ConfigTestElement
    implements TestBean, LoopIterationListener, NoConfigMerge {
    private static final Logger log = LoggerFactory.getLogger(UniqueCSVDataSet.class);
    ...
    private transient String threadGroup;
    
    private transient int blockSize = 0;
    
    public static Map<Integer, Set<String>> specificThreadGroupSetMap = new ConcurrentHashMap<>();
    
    private List<String[]> lineValuesList = new ArrayList<>();
    ...
    public void setProperty(JMeterProperty property) {
        ...
        if (propValue.contains(" ")) { // variables are unlikely to contain spaces, so most likely a translation
            try {
                ...
                for (String resKey : UniqueCSVDataSetBeanInfo.getShareTags()) {
    ...
    public void iterationStart(LoopIterationEvent iterEvent) {
    	...
    	if (isTargetThread == NO_TARGET) {	// not target
    		return;
    	}

		final JMeterContext context = getThreadContext();

    	if (isTargetThread == UNDEFINED) {		// not define
            String mode = getShareMode();
        	
        	if (mode.equals("shareMode.target")) {
                String threadGroupName = context.getThreadGroup().getName();
                Integer csvComponentHash = this.hashCode();
                
                if (checkProcess(csvComponentHash, threadGroupName)) {
                	isTargetThread = TARGET;	// target
                } else {
                	isTargetThread = NO_TARGET;	// not target
                	return;
                }
        	} else {
        		isTargetThread = TARGET;
        	}
    	}
    	
		JMeterVariables threadVars = context.getVariables();
    	
    	if (fillCnt >= blockSize ) {
            for (int i = 0; i < vars.length && i < lineValuesList.get(curIdx).length; i++) {
                threadVars.put(vars[i], lineValuesList.get(curIdx)[i]);
                curIdx++;
                if (curIdx >= blockSize) {
                	curIdx = 0;
                }
    ...
    private void setAlias(final JMeterContext context, String alias) {
        String mode = getShareMode();
        int modeInt = UniqueCSVDataSetBeanInfo.getShareModeAsInt(mode);
        switch(modeInt){
            case UniqueCSVDataSetBeanInfo.SHARE_ALL:
                this.alias = alias;
                break;
            case UniqueCSVDataSetBeanInfo.SHARE_GROUP:
                this.alias = alias + "@" + System.identityHashCode(context.getThreadGroup());
                break;
            case UniqueCSVDataSetBeanInfo.SHARE_THREAD:
    ...
    public boolean checkProcess(Integer csvComponentHash, String threadGroupName) {
		Set<String> specificThreadGroupSet = specificThreadGroupSetMap.get(csvComponentHash);
		
		if (specificThreadGroupSet == null) {
			specificThreadGroupSet = new HashSet<String>();
	    	String[] sharingThreadGroups = threadGroup.split(",");
	    	for (String sharingThreadGroup : sharingThreadGroups) {
	    		specificThreadGroupSet.add(sharingThreadGroup.trim());
	    	}
	    	specificThreadGroupSetMap.put(csvComponentHash, specificThreadGroupSet);
		}
		return (specificThreadGroupSet.contains(threadGroupName));
	}
    ...
    public void setThreadGroup(String value) {
